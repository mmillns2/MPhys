...
// Loops through each file
  for(size_t i{ 0 }; i < inputFiles.size(); ++i) 
  {
    ...
    for(Long64_t eventIndex = 0; eventIndex < N; eventIndex++)
    {
      ...
      double kaon3MomMag{kaon3Mom.length()};

      minLength[i] = std::min(minLength[i], kaon3MomMag);
      maxLength[i] = std::max(maxLength[i], kaon3MomMag);
    }
    ...
    for(Long64_t eventIndex = 0; eventIndex < N; eventIndex++)
    {
      ...
      W_vals[i].push_back(std::make_pair(kaon3MomMag, weight)); 
    }
    // File cleanup
    delete e;
    f->Close();
  ...
  // Generate histograms
  TH1D* W_1 = new TH1D("W1", "Kaon Momentum Distribution for Mono-energetic Neutrinos Incident on Nuclei;{p_K} [MeV];#sigma [cm^{2}]", bins, minLength[0], maxLength[0]);
  TH1D* W_2 = new TH1D("W2", "Kaon Momentum Distribution for Mono-energetic Neutrinos Incident on Nuclei;{p_K} [MeV];#sigma [cm^{2}]", bins, minLength[1], maxLength[1]);
  TH1D* W_3 = new TH1D("W3", "Kaon Momentum Distribution for Mono-energetic Neutrinos Incident on Nuclei;{p_K} [MeV];#sigma [cm^{2}]", bins, minLength[2], maxLength[2]);
  TH1D* W_4 = new TH1D("W4", "Kaon Momentum Distribution for Mono-energetic Antineutrinos Incident on Nuclei;{p_K} [MeV];#sigma [cm^{2}]", bins, minLength[3], maxLength[3]);
  TH1D* W_5 = new TH1D("W5", "Kaon Momentum Distribution for Mono-energetic Antineutrinos Incident on Nuclei;{p_K} [MeV];#sigma [cm^{2}]", bins, minLength[4], maxLength[4]);
  TH1D* W_6 = new TH1D("W6", "Kaon Momentum Distribution for Mono-energetic Antineutrinos Incident on Nuclei;{p_K} [MeV];#sigma [cm^{2}]", bins, minLength[5], maxLength[5]);
  TH1D* W_7 = new TH1D("W7", "Kaon Momentum Distribution for the DUNE FHC_FD_Flux Beam Incident on Argon;{p_K} [MeV];d#sigma [cm^{2}]", bins, minLength[6], maxLength[6]);
//  TH1D* hW_model_7 = new TH1D("W 7", "Kaon Momentum Distribution;{p_K} [MeV];d#sigma [cm^{2}]", bins, minLength, maxLength);
  ...
  // Output
  cW_14anti->Update();
  cW_14anti->Print("Pk_anti_14.png");
  ...
  // Output
  cW_DUNE->Update();
  cW_DUNE->Print("Pk_plot_DUNE.png");
