...
// Loops through each file
  for(size_t i{ 0 }; i < inputFiles.size(); ++i) 
  {
    ...
    for(Long64_t eventIndex = 0; eventIndex < N; eventIndex++)
    {
      ...
      minQ2[i] = std::min(minQ2[i], q2);
      maxQ2[i] = std::max(maxQ2[i], q2);
    }
    ...
    for(Long64_t eventIndex = 0; eventIndex < N; eventIndex++)
    {
      ...
      W_vals[i].push_back(std::make_pair(q2, bins*weight / (maxQ2[i] - minQ2[i]))); 
    }
    // File cleanup
    delete e;
    f->Close();
  ...
  // Generate histograms
  TH1D* W_1 = new TH1D("W1", "Q^{2} Distribution for Mono-energetic Neutrinos Incident on Nuclei;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[0], maxQ2[0]);
  TH1D* W_2 = new TH1D("W2", "Q^{2} Distribution for Mono-energetic Neutrinos Incident on Nuclei;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[1], maxQ2[1]);
  TH1D* W_3 = new TH1D("W3", "Q^{2} Distribution for Mono-energetic Neutrinos Incident on Nuclei;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[2], maxQ2[2]);
  TH1D* W_4 = new TH1D("W4", "Q^{2} Distribution for Mono-energetic Antineutrinos Incident on Nuclei;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[3], maxQ2[3]);
  TH1D* W_5 = new TH1D("W5", "Q^{2} Distribution for Mono-energetic Antineutrinos Incident on Nuclei;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[4], maxQ2[4]);
  TH1D* W_6 = new TH1D("W6", "Q^{2} Distribution for Mono-energetic Antineutrinos Incident on Nuclei;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[5], maxQ2[5]);
  TH1D* W_7 = new TH1D("W7", "Q^{2} Distribution for the DUNE FHC_FD_Flux Beam Incident on Argon;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2[6], maxQ2[6]);
//  TH1D* hW_model_7 = new TH1D("W 7", "Q^{2} Distribution;Q^{2} [MeV^{2}];d#sigma/dQ^{2} [cm^{2}MeV^{-2}]", bins, minQ2, maxQ2);
  ...
  // Output
  cW_14anti->Update();
  cW_14anti->Print("Q2_anti_14.png");
  ...
  // Output
  cW_DUNE->Update();
  cW_DUNE->Print("Q2_plot_DUNE.png");
