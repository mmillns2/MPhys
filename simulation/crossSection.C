...
public:
																	 ...
																	 std::string& data) const
	{
		for(size_t i{ 0 }; i < N; i++)
		{
			...
			stream<<std::scientific<<std::setprecision(10)<<yVals[i];
...
void crossSection()
{
	...
	double diffStep{ (diffMax - diffMin) / diffN };
	...
	std::array<double, diffN> fixedParameters;
	{
		...
		for(size_t i{ 0 }; i < diffN; i++)
		{
			pool.enqueue([&, i]()
			{
				...
				double diffCrossSection{ event.E3DiffCrossSection(fixedParameter, 20, 20, 20) };
				diffCrossSections[i] = diffCrossSection * 3.89739e-28; // cm^2
				fixedParameters[i] = fixedParameter;
			});
		}
	}
	std::string diffData;
